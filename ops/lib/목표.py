"""목표 — 목적 다섯(규칙 준수 · 맥락 유지 · 토큰 절약 · 정보 추적성 · 좋은 설명)을 기록에서 재고, 경보를 낸다 (설계 1절, 2026-09-17).

    ops 목표              재는 것 표 + 시뮬 마지막 판 + 경보
...
답 끝 판정이 대부분 걸림 · 규칙별 위반이 지난 주의 두 배 · 목적이나 시험이 빠진 규칙 · 어긋남 목록 · 시뮬에서 모델 둘 이상이 실패한 시나리오.
"""
...
def 재기(뿌리: Path, c: dict | None = None) -> tuple[list[tuple[str, str, str, str]], dict]:
    """목적마다 재는 것을 (이번 주, 지난 주) 로. 둘째 값은 경보가 쓰는 중간값."""
    import 판정
    c = c or 설정(뿌리)
    ...
    줄.append(("규칙 준수", "답 끝 판정을 시킨 번 가운데 걸린 번", f"{걸린번}/{판정번}", f"{걸린지난}/{판정지난}"))
...
def 경보(뿌리: Path, c: dict | None = None, 중간: dict | None = None) -> list[str]:
    ...
    if 중간["판정번"] and 중간["걸린번"] / 중간["판정번"] >= 0.7:
        나온것.append(f"답 끝 판정을 시킨 {중간['판정번']}번 가운데 {중간['걸린번']}번이 걸렸다 — 규칙이 안 지켜지거나 판정이 지나치다. `ops 횟수` 로 규칙별로 본다")
...
def 보이기(뿌리: Path, c: dict | None = None) -> str:
    c = c or 설정(뿌리)
    줄, 중간 = 재기(뿌리, c)
    나온것 = ["목적 다섯을 기록에서 잰 것 (memory/횟수.jsonl · 판정.jsonl · 문맥.jsonl · 어긋남 목록). 재는 법은 설계 1절.", "",
            "| 목적 | 재는 것 | 이번 주 | 지난 주 |", "|---|---|---|---|"]
