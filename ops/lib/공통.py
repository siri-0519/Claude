...
기본설정 = {
    ...
    "판정": {"모델": "haiku", "명령": "claude", "시간": 300, "끄기": False, "묶음": 20,
           "답규칙넣기": True},    # 「답할 때」 규칙을 세션의 첫 물음 직전에 한 번 문맥에 넣는다. 2026-09-17 시뮬로 견주니 판정이 건 문장 21 → 9, 세션마다 4,415 바이트 — 사용자가 같은 날 기본값을 켜라고 했다
    ...
    # 사용자가 쓰지 말라고 한 말과 대신 쓸 말 — 답과 문서에서 기계가 막는다 (기계 규칙 「안쓰는말」, 2026-09-17). 항마다 말 · 찾기(정규식, 없으면 말 그대로) · 대신.
    # 첫 항은 사용자가 2026-09-14 (「재보세요 · 재지 않은 · 안 재고」) 와 2026-09-17 에 쓰지 말라고 한 말의 활용꼴 전부다. 앞에 한글이 붙은 것(현재는 · 존재는)과 재검사 · 재기록 같은 다른 낱말은 잡지 않는다.
    "안쓰는말": [{"말": "잰다", "대신": "센다 · 본다 · 확인한다",
               "찾기": r"(?<![가-힣])(잰|잴|잽니|재는|재어|재다|재지(?![가-힣])|재면|재서(?![가-힣])|재려|재기(?![가-힣])|재고(?![가-힣])|재 ?본|재 ?보(?![가-힣])|재 ?봐)"}],
    "기본브랜치": "main",
    "세션브랜치": "claude/",
}
...
def 크기기록(자리: str, 글: str, 어떻게: str) -> None:
    """훅이 문맥에 넣거나(문맥) 막으며 보낸(막기) 글의 크기 한 줄 — 설계.md 1절 「토큰 절약」에서 세는 것(시작과 답마다 들어간 바이트).
...
# 횟수: 어긴 것 한 줄 · 판정: 판정을 한 번 시킬 때마다 한 줄(어긴 것의 분모) · 문맥: 훅이 문맥에 넣거나 막으며 보낸 글의 크기 한 줄과 Read 한 파일의 바이트 한 줄(토큰 절약에서 세는 것)
