...
def 물음(요청: str, 답: str, 규칙: str, 바뀐파일: list[str], 낱말후보: list[str], 자리: str) -> str:
    """판정 모델에게 주는 물음(훅-글 A18). 규칙 5 줄은 2026-09-17 에 좁혔다 — 이번 주 판정이 규칙 5 로 건 49 문장 가운데 29 가 근거가 같은 답에
    있거나 직접 본 값이었고(부록 C), 실제 답 넷을 옛 글과 새 글로 haiku · sonnet · opus 에게 판정시키니 규칙 5 걸림이 12 에서 5 로 줄고 남은 것은
    근거 없는 판정 문장이었다."""
    머리 = ("아래 규칙으로 아래 글을 판정한다. 어긴 규칙과 어긴 문장만 JSON 으로 돌려준다.\n"
            ...
            "글 전체를 읽고 판정한다. 한 문장에 빠진 근거나 표시가 같은 답 안 어디에든 있으면 어긴 것이 아니다. "
            "명령 결과 · 파일 · 기록에서 읽은 값(커밋 번호 · 파일 이름 · 수 · 시간)을 적은 문장은 직접 본 것이라 걸지 않는다. "
            "「직접 본 것 · 추측 · 크기를 밝힌다」는 세상의 사실이나 판정을 근거 없이 말하는 문장에만 건다. "
            "에이전트가 한 일 · 할 일 · 되묻는 물음 · 사용자에게 고르라는 항목을 말하는 문장에는 걸지 않는다. "
            "따옴표(\"…\" · 「…」) 안은 사용자나 문서의 말을 그대로 옮긴 것이라, 그 안의 낱말이나 상대 날짜로는 걸지 않는다. "
    ...
    if 자리 == "답":
        ...
        # 커밋 직전에는 요청이 없다. 「확인받은 목적만큼 고친다」는 요청과 확인받은 목적을 봐야 판정할 수 있어서, 요청이 없으면 걸지 않는다 —
        # 진행 상태 한 줄을 고친 것을 "요청 없이 줄을 더했다"고 잡았다 (2026-09-14 실측).
        부분 += ["=== 사용자의 요청 (있으면) ===", 요청 or "(없음)", "=== 문서에 새로 들어가거나 바뀐 줄 ===", 답,
                 "요청이 (없음)이면 「확인받은 목적만큼 고친다」는 판정할 수 없으므로 걸지 않는다. "
                 "줄이 새로 들어가거나 바뀐 것 자체는 어느 규칙도 어긴 것이 아니다. "
                 "「틀린 줄은 이어진 것까지 고친다」와 「누구의 말인지 표시한다」로 본다."]
    return "\n\n".join(부분)
...
def 되돌리는말(어긴것: list[dict], 뿌리: Path | None = None) -> str:
    ...
    줄 = ["방금 쓴 답이 판단 규칙에 걸렸다. 판단 규칙은 다른 모델이 판정한 것이다. "
          "걸린 문장마다 「무엇을 왜 고쳤다」 한 줄을 적고 그 아래에 고친 문장을 낸다. 고치지 않은 문장은 다시 붙이지 않는다 — 답 전체를 다시 내면 사용자에게 같은 답이 두 번 보이고, 고친 문장만 내면 앞뒤 없는 문장으로 보인다 (2026-09-17)."
          "걸린 것이 문장 하나가 아니라 여러 문장의 모양(견줄 것을 문장으로 늘어놓음 · 차례를 한 문단에 이음)이면, 그 문장들 대신 표 하나나 번호 줄을 낸다."]
    for x in 어긴것:
        번호 = x['규칙']
