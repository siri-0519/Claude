...
def 물음직전(뿌리: Path, c: dict, p: dict) -> int:
    import 판정
    n, 새로 = 어긋남개수(뿌리, c)
    글 = [어긋남한줄(n, 새로)] if n else []
    if (c.get("판정") or {}).get("답규칙넣기"):            # 설정으로 켠다 — 기본은 끔. 2026-09-17 시뮬로 켠 것과 끈 것을 견준다
        s = 상태읽기(뿌리, 세션파일)
        if "답할 때" not in (s.get("규칙넣음") or []):
            규칙 = 판정.규칙글(뿌리, "답할 때")
            if 규칙:
                s.setdefault("규칙넣음", []).append("답할 때")
                상태쓰기(뿌리, 세션파일, s)
                글.append(보낸이 + "답할 때의 판단 규칙이다. 답이 끝날 때 다른 모델이 이 규칙으로 답을 판정하고, 걸리면 그 문장을 고쳐 다시 내게 된다 — "
                         "쓰기 전에 알면 그 되돌림이 줄어든다. 이 세션에 지금 한 번만 넣는다.\n\n" + 규칙)
    if 글:
        문맥출력("user_prompt_submit", "\n\n".join(글))
    return 0
...
def 도구직전(뿌리: Path, c: dict, p: dict) -> int:
    ...
    if r == "CLAUDE.md":
        ...
        if 문제:
            return 막기("\n".join(문제))
    if 기계.안쓰는말파일인가(r, c):                  # 사용자가 쓰지 말라고 한 말 — 규칙 · 훅 글 · 목차 · 설명서까지 새 줄에서 막는다 (2026-09-17)
        문제 = [f"{r} {x}" for x in 기계.안쓰는말_새줄(새글, c, 지금글)]
        if 문제:
            return 막기("저장하기 전에 걸렸다 — 사용자가 쓰지 말라고 한 말이 새 줄에 있다. 아래 줄에서 그 말을 지우고 그 자리에 화살표 뒤의 말 가운데 뜻에 맞는 것을 적은 뒤 "
                      "같은 저장을 다시 한다. 다른 줄은 손대지 않는다. 사용자의 말을 따옴표 안에 그대로 옮긴 것이면 고치지 않고 따옴표로 감싼다.\n" + "\n".join(문제))
...
def 읽기기록(뿌리: Path, p: dict) -> None:
    """Read 마다 어느 파일을 몇 바이트 읽었나 — 토큰 절약에서 세는 것(설계 1절)에 빠져 있던 것. 2026-09-03 creation 에서 사용자가 손으로 센
...
되돌림모양 = ("걸린 문장마다 「무엇을 왜 고쳤다」 한 줄을 적고 그 아래에 고친 문장을 낸다. 고치지 않은 문장은 다시 붙이지 않는다 — "
          "답 전체를 다시 내면 사용자에게 같은 답이 두 번 보이고, 고친 문장만 내면 앞뒤 없는 문장으로 보인다 (2026-09-17).")
...
def 답끝(뿌리: Path, c: dict, p: dict) -> int:
    ...
    if 답 and not 되돌린뒤:
        ...
        if 걸린:
            ...
            return 막기(f"답에 상대 날짜가 있다: {' '.join(걸린)} — 이 낱말을 지우고 그 자리에 날짜를 적는다. "
                       f"오늘은 {오늘()} 이니, 그 일이 오늘 있었으면 {오늘()} 이라고 적는다. 사용자나 문서의 말을 따옴표(\"…\" · 「…」) 안에 그대로 옮긴 것이면 "
                       f"고치지 않고 따옴표로 감싼다. " + 되돌림모양)
        걸린말 = 기계.안쓰는말(답, c)
        if 걸린말:
            횟수추가(뿌리, "기계", "안쓰는말")
            return 막기("답에 사용자가 쓰지 말라고 한 말이 있다: " + " / ".join(걸린말) + " — 그 말을 지우고 그 자리에 화살표 뒤의 말 가운데 뜻에 맞는 것을 적는다. "
                       "사용자의 말을 따옴표(\"…\" · 「…」) 안에 그대로 옮긴 것이면 고치지 않고 따옴표로 감싼다. " + 되돌림모양)
        규칙 = 판정.규칙글(뿌리, "답할 때")
        if 규칙 and len(답) > 80:
    ...
    if 문제 and int(s.get("되돌림") or 0) < 2:
        ...
        return 막기("답을 끝내기 전에 커밋하고 push 한다. 이번 요청의 변경은 커밋 하나로 묶고, 메시지 첫 줄은 이번 요청의 핵심이다. "
                  "고친 것이 없으면 커밋은 만들지 않되, 브랜치가 원격에 없으면 그래도 push 한다(git push -u origin <브랜치>) — 커밋할 것이 없는 것은 push 를 안 할 이유가 아니다. "
