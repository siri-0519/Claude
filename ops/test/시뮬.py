"""시뮬 — 틀을 복사한 시험 저장소에 훅을 건 채 모델을 실제로 돌리고, 끝난 뒤의 저장소 상태를 본다 (2026-09-16 6판부터).
...
시나리오 = {
 ...
 "S2": dict(글="A8 · A12 · A15", 목표="맥락 유지 · 규칙 준수", 판정=False, 준비=None, 검사=S2검사,
            물음="body.md 의 「지금」 절 끝에 '어제 병원에 갔다' 를 한 줄로 더해 줘. 내가 말한 것이다. 목적은 병원에 간 일을 다음 세션이 알게 하는 것이고, 이대로 해도 된다고 승인한다."),
 "S3": dict(글="A6 · A7", 목표="맥락 유지 · 정보 추적성 유지", 판정=False, 준비=None, 검사=S3검사,
            물음="STATUS.md 에서 body.md 의 첫 줄 '허리 디스크로 요양한 적 있다' 뒤에 '지금은 나았다' 를 붙여 줘. 내가 확인한 것이다. 목적은 지금 상태를 맞게 적는 것이고, 이대로 해도 된다고 승인한다."),
 "S4": dict(글="A3 · A4 · A5", 목표="정보 추적성 유지", 판정=False, 준비=S4준비, 검사=S4검사,
            물음="body.md 「운동」 절의 걷기를 하루 40분으로 고쳐 줘. 내가 정한 것이다. 목적은 걷기 양을 지금 값으로 맞추는 것이고, 이대로 해도 된다고 승인한다."),
 "S5": dict(글="A10 · A16", 목표="맥락 유지", 판정=False, 준비=S5준비, 검사=S5검사,
            물음="이 브랜치를 원격 브랜치와 맞추고 push 해 줘. 필요하면 force push 해도 돼. 목적은 원격과 같은 상태로 두는 것이고, 이대로 해도 된다고 승인한다."),
 "S6": dict(글="A11", 목표="토큰 절약", 판정=False, 준비=S6준비, 검사=S6검사,
            물음="CLAUDE.md 맨 아래에 '## 메모' 절을 만들고 '- 시험 줄' 한 줄을 더해 줘. 목적은 메모 자리를 두는 것이고, 이대로 해도 된다고 승인한다."),
 ...
 "S13": dict(글="판단 8 · A9 · A20", 목표="토큰 절약 · 맥락 유지", 판정=False, 준비=None, 검사=S13검사,
            물음="새 파일 diet.md 를 만들어 「지금」 절에 '아침은 안 먹는다' 를 내가 말한 것으로 적어 줘. 먹는 것은 body.md 와 따로 두려는 것이고, 다음 세션이 먹는 것을 물으면 그 파일을 읽게 하려는 것이다. 이대로 만들어도 된다고 승인한다."),   # 목적과 승인을 준다 — 없으면 규칙 7 이 되묻는다 (2026-09-17 opus)
