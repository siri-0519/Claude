...
def main() -> int:
    ...
    확인(code == 2 and "스크립트가 만든다" in err, "생성 파일 Write 를 막는다")
    ...
    확인(code == 2 and "생성 블록" in err, "CLAUDE.md 생성 블록 안의 Edit 를 막는다")
    ...
    확인(code == 0 and (r / "guide.md.meta.yml").is_file(), "ops 출처 가 사이드카를 만든다")
    글, 열쇠 = 어긋남.목록(r, c)
